...
selType.onchange = async () => {
    const svar = await nettsideColl.where("Farge","==",selType.value).where("Type", "==", "Yttertøy").get();
    nettsideDiv.innerHTML = ``;
...
selType.onchange = async () => {
    ...
    if(selType.value === "alle"){
        ...
        svar = await nettsideColl.where("Farge","==",selType.value).where("Type", "==", "Yttertøy").get();
        nettsideDiv.innerHTML = ``;
        for (const nettside of svar.docs) {
            lagHTML(nettside.data(),nettside.id);
        }
